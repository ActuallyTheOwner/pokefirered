    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (((IS_MOVE_PHYSICAL(gCurrentMove) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT) && !gSaveBlock2Ptr->optionsBattleSceneOff) || 
                    ((IS_TYPE_PHYSICAL(moveType) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT) && gSaveBlock2Ptr->optionsBattleSceneOff))
                {
                ...
                //Split or not
                else if (((IS_MOVE_SPECIAL(gCurrentMove) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE)) && !gSaveBlock2Ptr->optionsBattleSceneOff) ||
                         ((!IS_TYPE_PHYSICAL(moveType) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE)) && gSaveBlock2Ptr->optionsBattleSceneOff))
                {
